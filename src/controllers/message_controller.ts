...
export const messageController = {
    ...
    getUnreadMessageCount: async (req: Request, res: Response) => {
        ...
        try {
            ...
            res.status(200).json({ message: "unread messages counts", unreadMessages: unreadCount });
        } catch (error) {
            console.error(error);
            res.status(500).json({ message: "Failed to get unread message count", error });
        }
    },
        ...
        try {
            ...
            messages.forEach((message) => {
                ...
                if (!chatMap.has(otherUserId)) {
                    chatMap.set(otherUserId, {
                        ...
                        unreadCount: message.receiverId.toString() === userId.toString() && message.status === "sent" ? 1 : 0,
                    ...
                    if (message.receiverId.toString() === userId.toString() && message.status === "sent") {
            ...
            const chatList = await Promise.all(
                Array.from(chatMap.values()).map(async (chat) => {
                    const user = await userSchema.findById(chat.userId).select("full_name avatar");
                    const isOnline = onlineUsers.has(chat.userId);

                    // Truncate the last message if it exceeds the maximum length
                    const truncatedLastMessage = chat.lastMessage && chat.lastMessage.length > maxLastMessageLength
